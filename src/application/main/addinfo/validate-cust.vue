...
<template>
  <div id="applicantDateCust">
    <row-2-col>
      ...
      <template  slot="val">
        <div class="right">
          <za-date @onChange="onBeginDateChange" class="begindate" :formModel="begindateModel" :inset="true" :index="index" name="begindate">
          </za-date>
          <!-- <label for="sameappl">
            <input type="checkbox" class="weui-check" id="sameappl" checked="checked" @change="click" name="check">
            <i class="icon-checked"></i>
             <span class="title">长期</span>
          </label> -->
        </div>
      </template>
    </row-2-col>
