<template>
   <div class="cust-item" v-if="benefitType==1">
        ...
        <ul>
          <div>
            <ul class="wrap" v-if="custinfo.benefictaryDTO">
              ...
              <router-link :to="{path: 'beneficiary-info', query: {applyId: applyId, insureSeq: custinfo.insureSeq, benefictaryType: current, planId: planId}}" v-if="current==2">
...
export default {
  ...
  mounted(){
    ...
    this.benefitType = this.custinfo.benefitType
    if(this.custinfo.relation == "00"){
       this.tabs.pop()
    }
    this.$nextTick(() => {
