...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      View (pid, aid) {
        this.$router.push({name: '/web/progressinfo', query: {applyId: aid, planId: pid}})
      },
      Edit (s, planId, applyId, isShowBeneficiary) {
        const query = {planId, applyId}
        // 1 客户信息
// 2 产品信息
// 3 保险计划
// 4 STEP_PREW
...
// 100 投保单流程已结束
        switch (s) {
          case 1:
          this.$router.push({path: "/insure", query})
          break
          case 2:
          this.$router.push({name: "app-feature", query})
          break
          case 3:
          this.$router.push({name: "make-plan", query})
          break
          case 4:
          this.$router.push({path: "/viewplan", query})
          break
          case 10:
          this.$router.push({path: "/addinfo", query})
          break
          case 11:
          this.$router.push({path: "/beneficiary", query})
          break
          case 12:
          this.$router.push({path: "/addinfo", query})
          break
          case 13:
          this.$router.push({path: "/notify/applicant", query})
          break
          case 14:
          this.$router.push({path: "/notify/insurant", query})
          break
          case 15:
          this.$router.push({path: "/notify/grade", query})
          break
          case 16:
      ...
      getData (append = true) {
        ...
        var param = {
          applicantName: this.$route.query.name || '',
          genderCode: this.$route.query.genderCode || '',
          birthday: this.$route.query.birthDay || '',
          certType: this.$route.query.certType || this.$route.query.idcode || '',
          certNo: this.$route.query.certNo || this.$route.query.idnumber || '',
          tabId: this.idcode,
...
//        }).catch((err) => {
      ...
      Click (n) {
        this.$router.push({name: '/web/progressinfo', query: this.$router.query})
      }
    }
  }
  </script>
