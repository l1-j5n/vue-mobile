  ...
  <div class='page-progress'>
    <div>
      <tab :animate="false" :line-width="2" active-color="#14a3ff" defaultColor="#666">
        <tab-item v-for="(item,index) in tab" :key="item.id" :selected="seled == index" @on-item-click="handler(index, item.id)">
          {{item.name}}
        </tab-item>
      </tab>
    </div>
...
<script>
  ...
  import {ISEARCHLIST,INSBENEFICARYINFOR, PROGRESSTAB, INPROGRESSLIST} from '@/api'
...
//  import progressbox from './progressbox'
  ...
  export default {
    ...
    created: function () {
      window.gotosearchprogproc = function () {
        this.$router.push({path: '/searchprogproc'});
      }
...
<style lang='less' scoped>
  ...
  /*@include display-flex;*/
    ...
    /*.active{*/
      /*border-bottom: 3px solid #00a6ff!important;*/
